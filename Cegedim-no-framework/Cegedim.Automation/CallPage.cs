...
namespace Cegedim.Automation {
    ...
    public class CallPage : CegedimPage {
        ...
        internal CallPage(MITouch application, AppContainer container)
    ...
    public class Calendar : CegedimSubPage {
        ...
        internal Calendar(MITouch application, CegedimEntity parent, AppElement element) 
                ...
                Func<bool> tapToYear = () => {
                    ...
                    if (currentYear != value) {
                        if (value > currentYear) {
                            TapAndWait(Query.NextYear, () => currentYear != Year, postTimeout: TimeSpan.FromSeconds(0.2));
                        } else {
                            TapAndWait(Query.PreviousYear, () => currentYear != Year, postTimeout: TimeSpan.FromSeconds(0.2));
                        }
                        return false;
                    } else {
                        return true;
                ...
                Func<bool> tapToMonth = () => {
                    ...
                    if (currentMonth != value) {
                        if (monthTable[currentMonth] < monthTable[value])
                            TapAndWait(Query.NextMonth, () => currentMonth != Month, postTimeout: TimeSpan.FromSeconds(0.2));
                        else
                            TapAndWait(Query.PreviousMonth, () => currentMonth != Month, postTimeout: TimeSpan.FromSeconds(0.2));
                        return false;
                    } else {
                        return true;
                    }
        ...
        public int Day {
            ...
            set {
                ...
                try {
                    ...
                    if (TestIsVisible(validDayQuery + " index:1"))
                        TapAndWait(validDayQuery + " index:1", () => Calabash.Query("*") != initialQuery);
                }
                m_day = value;
            }
        }
